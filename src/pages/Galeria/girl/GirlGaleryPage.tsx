...
export const GirlGaleryPage = () => {
  ...
  const { data: selectedGaleryDetail, refetch: refetchSelectedDetail } = useQuery({
    queryKey: ['girlGaleryDetailSelected', selectedGalery],
    queryFn: () => girlGaleryService.getById(selectedGalery!),
  ...
  const handleCreateGalery = async (name: string, image: File | null) => {
    try {
      ...
      if (image && result.id) {
        await girlGaleryService.uploadImage(image, result.id);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(files)) {
      try {
        await girlGaleryService.uploadMedia(file, selectedGalery);
      } catch (error) {
  ...
  const handleSaveLinks = async () => {
    ...
    try {
      await girlGaleryService.updateLinks(selectedGalery, links);
  ...
  const handlePaste = useCallback(async (e: ClipboardEvent) => {
    ...
    for (let i = 0; i < items.length; i++) {
      if (items[i].type.indexOf('image') !== -1) {
        ...
        try {
          if (selectedGalery) {
            await girlGaleryService.uploadMedia(blob, selectedGalery);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1
          ...
          onClick={() => selectedGalery && navigate('/galeria/girl')}
        >
          {selectedGaleryData?.name || 'Galeria Chicas'}
        </h1>
